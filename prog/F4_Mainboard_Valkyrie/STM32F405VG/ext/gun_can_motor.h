...
*/
void gun_can_motor_set_vel(gun_MotorID id, int32_t vel, CloseLoopFlag loop);
...
*/
void gun_can_motor_set_pos(gun_MotorID id, int32_t pos);
...
*/
void gun_can_motor_set_accel(gun_MotorID id, uint32_t accel);
...
*/
void gun_can_motor_set_max_vel(gun_MotorID id, uint32_t vel);

/**
* Lock the motor immediately with close loop.
* @param id: MOTOR_x, the motor to be controlled
*/
void gun_can_motor_lock(gun_MotorID id);
...
*/
void gun_can_motor_set_competition_mode(gun_MotorID id, FunctionalState new_state);
...
* @param state: ENABLE/DISABLE
*/
void gun_can_motor_stfu(gun_MotorID id, FunctionalState new_state);
...
*/
int32_t gun_can_get_encoder(gun_MotorID id);

/**
* Get the motor pwm value (based on CAN rx result)
* @param id: MOTORx, which motor to look at
*/
int32_t gun_can_get_pwm(gun_MotorID id);

/**
* Get the motor state (NO_RESPOND, RUNNING, or ENCODER_DEAD)
* @param id: MOTORx, which motor to look at
*/
GunMotorState gun_can_get_state(gun_MotorID id);

/**
* Get the limit switch triggered state
* @param id: MOTORx, which motor to look at
*/
bool gun_can_get_limit_trig(gun_MotorID id, GunLimitID limit_id);

/**
* Get the limit switch position
* @param id: MOTORx, which motor to look at
*/
s32 gun_can_get_limit_pos(gun_MotorID id, GunLimitID limit_id);
...
*/
void gun_reg_OnLimitPosUpdate(gun_MotorID id, OnLimitPosUpdate listener);
...
*/
void gun_reg_OnTargetReached(gun_MotorID id, OnTargetReached listener);

//Disabling the listener
void gun_disable_OnLimitPosUpdate(gun_MotorID id);
void gun_disable_OnTargetReached(gun_MotorID id);
