...
void garuda_init(GarPriority priority){
	...
	//spi_init(SPI_GAR_PORT, SPI_Mode_Slave, SPI_BaudRatePrescaler_64);// no spi 
	...
	//spi_rx_set_handler(SPI_GAR_PORT, spi_gar_rx_handler);// no spi
	spi_gar_update_ssi();
...
u32 gar_button_pressed_time(GarButtonID buttonId){
	return pressedTime[buttonId];
}
...
void gar_button_update(){
	...
	for (uint8_t i=0; i<GAR_BTN_COUNT; i++){
		if (connection != GAR_BOTH_DISCONNECTED){
			if (tar_btn_array[i]){
