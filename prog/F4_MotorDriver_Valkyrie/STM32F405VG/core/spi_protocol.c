...
u16 spi_tx_byte(SPI_TypeDef* port, u8 data) {
	while (!(*((volatile uint16_t*)(&port->SR)) & SPI_I2S_FLAG_TXE));
	*((volatile uint8_t *)(&port->DR)) = data;

	if (SpiRxListenerList[spi_get_id(port)] != 0) return 0;
	
	while (!(*((volatile uint16_t*)(&port->SR)) & SPI_I2S_FLAG_RXNE));
	return *((volatile uint16_t *)(&port->DR));
}

void spi_tx_only(SPI_TypeDef* port, u8 data) {
	while (!(*((volatile uint16_t*)(&port->SR)) & SPI_I2S_FLAG_TXE));
	*((volatile uint8_t *)(&port->DR)) = data;
}
